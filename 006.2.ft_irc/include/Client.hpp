...
/**
 * @class Client
 * @brief Represents a client connected to the IRC server.
 *
 * The `Client` class encapsulates the properties and behaviors of a client connected to the IRC server.
 * It provides methods to manage the client's socket, authentication status, nickname, username, real name,
 * host name, server name, and channels. It also allows sending and receiving messages to/from the client.
 */
class Client
{
private:
